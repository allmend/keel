...
- Config directory — `keel.yaml` plus per-team files for pools, vhosts and certificates; in a cluster, one versioned set replicated to every node
...
```bash
docker pull ghcr.io/allmend/keel:0.20.0
docker run -v /etc/keel:/etc/keel -p 80:80 -p 443:443 ghcr.io/allmend/keel:0.20.0
```
...
./target/release/keel --config node.yaml
```
...
Keel reads two files: the node file, `/etc/keel/node.yaml` — how this node runs — and the config directory it names, `/etc/keel/config/` — the load balancer. Minimal config: listen on port 80, proxy to two backends:
...
All inter-node traffic is mTLS and the join exchange itself is encrypted with a key derived from the shared secret. The cluster CA is generated by the bootstrap node and replicated to every member, so a new node can join through any of them. Joining nodes retry with backoff (safe to start all nodes at once) and are promoted to Raft voters once caught up. The config directory is replicated as versions (`keel config push` or `SIGHUP` on any node) and applied on every node, and ACME certificates are committed via Raft too; drain and reload act per node; `keel cluster stepdown` removes a node gracefully, refusing (without `--force`) when the remaining nodes would lose quorum.
