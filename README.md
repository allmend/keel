...
```bash
docker pull ghcr.io/allmend/keel:0.9.0
docker run -v /etc/keel:/etc/keel -p 80:80 -p 443:443 ghcr.io/allmend/keel:0.9.0
```
...
Keel is at v0.9.0, alpha quality. Core proxy, TLS + ACME, clustering, caching, TCP and UDP (L4) proxying, and protocol health checks with passive detection are implemented and working. See [CHANGELOG.md](CHANGELOG.md) for known limitations before deploying.
